...
void OLED_show_floatnum(uint8_t x, uint8_t y, float num, uint8_t size)
{
  ...
  chartemp[4] = ' ';
  chartemp[5] = '\0';
  OLED_ShowString(x, y + i, chartemp,1);
}
...
void LcdSprintf(uint8_t Colume, uint8_t Line, char *format,...)
{
    ...
    va_end(arg);
    OLED_ShowString(Colume, Line, (uint8_t*)String,1);
}
...
{
    uint8_t x, y;
    for (x = x0; x <= x1; x++)  // 绘制矩形的上边和下边
    {
        OLED_DrawPoint(x, y0, color);  // 上边
        OLED_DrawPoint(x, y1, color);  // 下边
    }

    for (y = y0; y <= y1; y++)  // 绘制矩形的左边和右边
    {
        OLED_DrawPoint(x0, y, color);  // 左边
        OLED_DrawPoint(x1, y, color);  // 右边
    }
}
